...
import com.perozzi_package.pokemontypecalculator.Effectiveness
import com.perozzi_package.pokemontypecalculator.R
import com.perozzi_package.pokemontypecalculator.adapters.TypeGrid
...
class TypeCalculatorViewModel(private val resources: Resources, private val app: Application) :
    ...
    var gridLayoutManager: GridLayoutManager? =
        GridLayoutManager(app, 3, LinearLayoutManager.VERTICAL, false)
    ...
    private fun effectivenessToCellTextColors(mutableList: MutableList<String>): MutableList<Int> {
        ...
        for (i in 0 until 18) {
            if ((mutableList[i] == Effectiveness.DOES_NOT_EFFECT.impact) || (mutableList[i] == Effectiveness.ULTRA_DOES_NOT_EFFECT.impact) || (mutableList[i] == Effectiveness.ULTRA_NOT_VERY_EFFECTIVE.impact)) {
    ...
    fun fetchJson() {
        ...
        client.newCall(request).enqueue(object : Callback {
            ...
            override fun onFailure(call: Call, e: IOException) {
                typeMatchups = backupData
            }
        })
    }

    val backupData: Map<Types, Map<String, Double>> = mapOf(
        Pair(
            Types.Bug, mapOf(
                Pair("Bug", 1.0),
                Pair("Dark", 1.6),
